...
class HookResponderTests(test.FrameworkTest):
  ...
  def test_bind_context(self):
    ...
    c = hooks.Context(('sample_item', 'sample_item2'), rollup=True)

    def sup(*args, **kwargs): return kwargs

    # bind context
    unwrapped = c(sup)
    assert 'sample_item' in c.__requested__ and 'sample_item2' in c.__requested__
  ...
  def test_bind_invalid_context(self):
    ...
    c = hooks.Context(('sample_item', 'sample_item2', 'sample_item3'), rollup=True)
